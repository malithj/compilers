...
std::pair<int, std::string> InputBuffer::getToken(NFA &nfa)
{
	...
	// The following loop will keep iterating until either the NFA has
	// run out of current states, or there are no more characters to read
	// from the input. At that point, it will be time to decide what
	// to do with the best lexeme (or lack of it) found so far.
